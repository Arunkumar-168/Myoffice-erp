...
import '../Pref/all_category.dart';
import '../Pref/sub_category.dart';
...
class Categories with ChangeNotifier {
  ...
  Future<void> fetchSubCategories(int catId) async {
    var url = BaseURL + 'sub_categories/$catId';
    try {
      final response = await http.get(Uri.parse(url));
      final extractedData = json.decode(response.body) as List;
      // ignore: unnecessary_null_comparison
      ...
      for (var catData in extractedData) {
        loadedCategories.add(SubCategory(
          ...
          number_of_courses: catData['number_of_courses'],
        ));
  ...
  Future<void> fetchAllCategory() async {
    var url = BaseURL + 'all_categories';
    try {
      final response = await http.get(Uri.parse(url));
      final extractedData = json.decode(response.body) as List;
      print(extractedData);
      ...
      for (var catData in extractedData) {
        loadedCategories.add(AllCategory(
          id: int.parse(catData['id']),
          title: catData['name'],
          subCategory: buildSubCategory(catData['sub_categories'] as List<dynamic>),
        ));
