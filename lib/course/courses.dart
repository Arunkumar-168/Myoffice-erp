...
import 'package:flutter_application_1/online/course.dart';
import 'package:flutter_application_1/online/course_detail.dart';
import 'package:flutter_application_1/online/lesson.dart';
import 'package:flutter_application_1/online/section.dart';
...
class Courses with ChangeNotifier {
  ...
  Future<void> fetchCoursesByCategory(int categoryId) async {
    var url = BaseURL + 'category_wise_course?category_id=$categoryId';
    try {
      final response = await http.get(Uri.parse(url));
      final extractedData = json.decode(response.body) as List;
      // ignore: unnecessary_null_comparison
  ...
  Future<void> fetchCoursesBySearchQuery(String searchQuery) async {
    var url =
        BaseURL + 'courses_by_search_string?search_string=$searchQuery';
    // print(url);
    try {
      final response = await http.get(Uri.parse(url));
      final extractedData = json.decode(response.body) as List;
  ...
  Future<void> filterCourses(
    ...
    var url = BaseURL +
        'filter_course?selected_category=$selectedCategory&selected_price=$selectedPrice&selected_level=$selectedLevel&selected_language=$selectedLanguage&selected_rating=$selectedRating';
    // print(url);
    try {
      final response = await http.get(Uri.parse(url));
      final extractedData = json.decode(response.body) as List;
  ...
  List<Course> buildCourseList(List extractedData) {
    ...
    for (var courseData in extractedData) {
      loadedCourses.add(Course(
        ...
        attachmentType: courseData['attachment_type'],
  ...
  Future<void> toggleWishlist(int courseId, bool removeItem) async {
    var authToken = await SharedPreferenceHelper().getAuthToken();
    var url = BaseURL +
        'toggle_wishlist_items?auth_token=$authToken&course_id=$courseId';
    if (!removeItem) {
  ...
  Future<void> fetchCourseDetailById(int courseId) async {
    var authToken = await SharedPreferenceHelper().getAuthToken();
    var url = BaseURL + 'course_details_by_id?course_id=$courseId';

    if (authToken != null) {
      url = BaseURL +
          'course_details_by_id?auth_token=$authToken&course_id=$courseId';
    }
