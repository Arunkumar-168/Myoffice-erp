...
class VdoCustomControllerView extends StatefulWidget {
  ...
  @override
  _VdoCustomControllerViewState createState() =>
      _VdoCustomControllerViewState();
}
...
class _VdoCustomControllerViewState extends State<VdoCustomControllerView>
  ...
  late EmbedInfo sample_1;// Declare selectedUrl here

  @override
  void initState() {
    super.initState();

    // Retrieve selectedUrl from route arguments
    selectedUrl =
    ...
    // Start timer if video is not in loading state
      ...
      child: GestureDetector(
        ...
        child: SizedBox(
          ...
          child: Stack(
            ...
            children: [
              Positioned.fill(
                child: (showProgressIndicator || showError)
                    ? Container(
                  color: Colors.black,
                  child: const Text(''),
                )
              ...
              showOverlayControls ? _vdoOverlayController() : const SizedBox.shrink(),
              Positioned(
                child: showOverlayControls ? _vdoProgress() : const SizedBox.shrink(),
                bottom: 30.0,
                left: 0,
                right: 0,
              ),
              Positioned(
                child: showOverlayControls ? _bottomControlPanel() : const SizedBox.shrink(),
                bottom: 6.0,
                left: 0,
                right: 0,
              ),
    ...
    return Center(
      child: Row(
        ...
        children: [
          ...
          InkWell(
            child: _playPauseRewind(),
            onTap: () => _playPauseRewindToggle(),
          ),
          InkWell(
            child: Transform(
  ...
  Widget _errorInfo() {
    ...
    return Column(
      ...
      children: [
        ...
        Text(
          'Error code: ${_playerValue.vdoError!.code}',
          style: const TextStyle(fontSize: 16, color: Colors.white),
        ),
        Text(
          _playerValue.vdoError!.message,
          style: const TextStyle(fontSize: 16, color: Colors.white),
        ),
      ],
    );
